...
class Postgresql {
	...
	*/
	async query(statement, params, transactionId) {
		let result = null;
		if (transactionId) {
			result = await this.transactionClients[transactionId].query(statement, params);
		} else {
			result = await this.pool.query(statement, params);
		}
		return result.rows;
	}
	...
	*/
	async insert(statement, transactionId) {
		const sql = statement + ' returning *';
		if (transactionId) {
			const client = this.transactionClients[transactionId];
	...
	*/
	update(statement, transactionId) {
		if (transactionId) {
			return this.transactionClients[transactionId].query(statement);
		}
	...
	*/
	async beginTransaction() {
		...
		this.transactionClients[id] = client;
	...
	*/
	async commitTransaction(uuid) {
		const client = this.transactionClients[uuid];
		if (client) {
			await client.query('COMMIT');
			client.release();
			delete this.transactionClients[uuid];
		}
	}
	...
	*/
	async rollbackTransaction(uuid) {
		const client = this.transactionClients[uuid];
		if (client) {
