 ...
 */
Models.prototype.restify = function (router, authentication, onLoadModel) {
...
Models.prototype.find = function find(Model, params, cb) {
	var self = this;
	var modelFind = Model.find !== undefined ? Model.find(params) : this.defaultFind(Model, params);
	...
	var columnDefinition = Model.columnDefinition !== undefined ? Model.columnDefinition() : null;
...
Models.prototype.findResponse = function(Model) {
	...
	return  function (req, res) {
	    self.find(Model, req.query, function (err, result) {
	    	if (err) {
	            res.json({
	                err: err
	            });
...
Models.prototype.defaultTitle = function(Model) {
	var str = Model.toString().split('function')[1];
	return str.substr(0,str.indexOf('()')).trim();
};

Models.prototype.defaultFind = function(Model, params) {
	var table = Model.tableName ? Model.tableName : this.defaultTitle(Model);
	var columns = Model.columnDefinition ? this.selectColumns(Model.columnDefinition()) : '*';
	var sqlExpress = this.addWhereExpress(Model, "select "+ columns +" from " + table, params, true);
