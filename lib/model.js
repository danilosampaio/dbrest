...
class Model {
	...
	async insert(params, transactionId) {
		const sqlExpress = await this.insertExpress(params);
		return this.database.insert(sqlExpress, transactionId);
	}

	async update(params, transactionId) {
		const sqlExpress = await this.updateExpress(params);
		return this.database.query(sqlExpress, null, transactionId);
	}

	async delete(params, transactionId) {
	...
	async sqlValue(name, def, value) {
		...
		if (type === undefined || type === 'text' || type === 'memo' || type === 'money') {
			...
			sqlValue = this.toBitStringValue(value);
		} else {
