...
class Model {
	...
	async getTableName(){
		if(this.tableName){
			...
			const results = await this.database.getTableName(this.defaultTitle());
			if(results && results.length > 0){
				return Object.values(JSON.parse(JSON.stringify(results[0])))[0];
			}else {
				return this.title;
			}
		}
	...
	async defaultSearchFields() {
		...
		return this.database.dialect === 'postgresql'
			? Object.keys(columns).map(c => columns[c].columnName)
			: Object.keys(columns);
	}
	...
	async selectColumns(definition) {
		const columns = await this.define();
		return this.database.dialect === 'postgresql'
				? Object.keys(_.pickBy(definition, c => c.type !== 'model')).map(c => columns[c].columnName)
				: Object.keys(_.pickBy(definition, c => c.type !== 'model'));
	}
	...
	async findExpress(params) {
		...
		const tableName = this.database.dialect === 'postgresql' ? `"${table}"` : table;
		const columns = await this.selectColumns(await this.define());
		const columnNames = this.database.dialect === 'postgresql'
			? columns.map(c => `"${c}"`)
			: columns;
		const sqlExpress = this.addWhereExpress(`select ${columnNames} from ${tableName} `, params, true);
